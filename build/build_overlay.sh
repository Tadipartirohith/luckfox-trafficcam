...
# Sanity checks
[ -d "$SDK/output/out/rootfs_uclibc_rv1106" ] || { echo "ERROR: rootfs not found in SDK. Run a full SDK build first."; exit 1; }
[ -d "$SDK/output/out/oem" ] || { echo "ERROR: OEM partition not found in SDK."; exit 1; }
which ${TOOLCHAIN_PREFIX}-gcc > /dev/null 2>&1 || export PATH="$SDK/tools/linux/toolchain/arm-rockchip830-linux-uclibcgnueabihf/bin:$PATH"
